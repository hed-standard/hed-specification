    ...
    for test_name in test_names:
        ...
        with open(file_name) as file:
            ...
            combined_data.extend(
                data
            )  # Assume each JSON file contains a list and concatenate
...
def main(exclude_names=None, out_name="temp.json"):
    ...
    filtered_files = [
        f
        for f in file_names
        if not any(f.startswith(prefix) for prefix in exclude_names)
    ]
